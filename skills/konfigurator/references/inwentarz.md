...
| czujka | `.claude/skills/czujka/` | `czujka/config.md` | 5 (K1 Zestaw ... K5 Kadencja) | Firecrawl (logowanie CLI albo `FIRECRAWL_API_KEY`); Meta Ads dodatkowo `APIFY_API_TOKEN` | `firecrawl --status` (albo `npx -y firecrawl-cli@latest --status`): linia "Authenticated" = jest; plus sprawdzenie Apify jak wyżej, gdy w `config.md` jest sekcja Reklamy z wpisami |
